...
Panel A-D show the mean [median] cumulative and buy-and-hold abnormal returns for each of the four event windows. Column (1) presents the abnormal returns for the complete sample, whereas column (2) and (3) show the abnormal returns for the sub-samples of strong and weak investors. The two sub-samples are the same as presented in Section X. Column (4) tests the difference in means [medians] of column (2) and (3). All returns presented in Table III are winsorized at the 1\% and 99\% level to equal out the effect of rare outliers.
Panel A presents the abnormal returns for the largest event window [-10+3], neglecting the possibility that the abnormal returns are also driven by other announcements. Both CAR and BHAR are positive and strongly significant at the 1\% level. Targets that belong to the sub-sample of filings submitted by strong investors have a mean CAR and BHAR 2.5\% higher when compared to weak investors. The difference in medians is around 7\% and strongly significant. 
