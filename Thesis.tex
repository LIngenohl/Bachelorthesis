...
\subsection{Market Returns to Initial 13(D) Filings -- Stock Returns}
% Intention
...
% Nature of AR's
In this sense, abnormal returns $AR_{i\tau}$ for security $i$ at event day $\tau$ are defined as the difference between the actual (realized) return $R_{i\tau}$ and the expected return $E(R_{i\tau}|X{\tau})$, given the absence of the event:
	\begin{equation}\label{eq:1}
		AR_{i\tau}=R_{i\tau}-E(R_{i\tau}|X_{\tau})
	\end{equation}
The the expected return in \eqref{eq:1} is calculated in the following ways: can be done in several ways. is unconditional on the event date, . Estimating the normal performance can be done with several models.
% Theory 
