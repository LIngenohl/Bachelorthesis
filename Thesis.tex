...
\section{Identifying the Investors}
The sample of the Schedule 13(D) filings consists only of filings submitted by corporations. But what characterizes them? Do they have common characteristics? Are there common characteristics the gener

\subsection{Financial Strength Measure}
In a study of 2010, \emph{BCG} noted many of that year's acquisitions would involve a financially strong acquirer. However, the attribute of being financially strong is not ambivalent. In order to analyse a companies condition, the following measure of fundamental analysis are conducted. profitability liquidity and operational performance  firm health performance

\subsubsection{Piotroski's F-Score}
With the aim to seperate strong from weak value firms, \citet{Piotroski2000} established the F-score. The score is the sum of nine binary signals that form a "... composite measure of firm strength" \citep[p. 496]{Fama2006} through a simple application of fundamental analysis. Furthermore the F-score considers in what directions the fundamentals of a company are trending and whether general health conditions are met \citep{Mohr2012}. 
	\footnote{In order to legitimize the explanatory power of the F-score in separating strong from weak firms he formed portfolios.In doing so he showed that an investment strategy of shorting expected losers (weak firms) and buying expected winners (strong firms) would "generate a 23\% average annual return" \citep[p. 4]{Piotroski2000}. \citet{Hyde2014} have matching results and observe significant return premiums for stock with a high F-score over stocks with a low F-score.}.
Although the F-score was established to distinguish among value firms, \citet{Mohr2012} shows that an application on growth stocks yields similar results without loosing the predictive ability 
	\footnote{This is in line with \citet{Piotroski2000} and confirms earlier research conducted by him}. 
This means that the F-Score can proxy as a financial strength measure in any case and seperates the sample of Schedule 13(D) filings into strong and weak investors. 

\subsection{Financial Distress measures}
Financial distress is a condition where a company cannot meet, or has difficulty paying off, its financial obligations to its creditors. HNA Group, for example, could be in a state of financial distress as they have problems refinancing the enormous debt burden. The following measures will be accounting based prediction models.

\subsubsection{Altman's Z-Score}
Altman's Z-score is a widely accepted measure of financial distress. The fundamental indicator shows statistically significant results in predicting the bankruptcy of a company \citep{Campbell2008} and is still applied as a general practical tool for assessing the financial well-being of firms \citep{Kleinert2014}. The model consists of five financial ratios that are coefficients by discriminate analysis method where the financial ratios are independent variables of it 
	\footnote{The five financial ratios of the Z-score are (1) working capital to total assets, (2) retained earning to total assets, (3) earnings before interest and taxes to total assets, (4) market value of equity to book value total debt and (5) sales to total assets. This yields
		\begin{equation}
...
Tobin's Q is included because it proxies for investment opportunity 
\pagebreak
